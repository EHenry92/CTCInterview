...
for (let key in funcs) {
  ...
  describe('ch4-q12: ' + key, function() {
    ...
    xit('throws an error if tree is null or empty', function() {
    ...
    xit('returns correct counts with unbalanced tree', function() {
      [10, 8, 16, 4, 14, 22, 6, 12, 18, 5, 17, 19].forEach(v => this.tree.add(v));
      expect(func(this.tree, 10)).to.equal(2);
      expect(func(this.tree, 18)).to.equal(3);
    });

    xit('returns correct counts with lots of paths that equal value', function() {
