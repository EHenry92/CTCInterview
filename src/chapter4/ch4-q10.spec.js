...
for (let key in funcs) {
  ...
  describe('ch4-q10: ' + key, function() {
    ...
    xit('throws an error if tree1 is null or empty', function() {
    ...
    xit('returns true where tree2 is null or empty', function() {
      this.tree1.add(1);
    ...
    xit('returns false with two different trees', function() {
    ...
    xit('returns true with balanced tree were it is subtree', function() {
      [8, 4, 12, 2, 6, 10, 14, 1, 3, 5, 7, 9, 11, 13, 15].forEach(v => this.tree1.add(v));
    ...
    xit('returns true when is a subtree but there exists multiple possible roots', function() {
    ...
    xit('returns false when tree2 is in tree1 but not a subtree (extra leaves)', function() {
      [1, 8, 4, 12, 2, 6, 10, 14, 1, 3, 5, 7, 9, 11, 13, 15, 2].forEach(v => this.tree1.add(v));
      [2, 1, 3].forEach(v => this.tree2.add(v));
      expect(func(this.tree1, this.tree2)).to.be.false;
    });

    xit('returns true with imbalanced tree were it is subtree', function() {
