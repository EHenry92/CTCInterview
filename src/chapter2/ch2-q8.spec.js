...
for (let key in funcs) {
  ...
  describe('ch2-q8: ' + key, function() {
    ...
    xit('returns null with empty list', function() {
      expect(func(this.list.head)).to.be.null;
    });

    xit('returns null when there is no loop', function() {
      helpers.push(this.list, 1, 2, 3, 4, 5, 6);
      expect(func(this.list.head)).to.be.null;
    });

    xit('returns node when there is a loop 1', function() {
      helpers.push(this.list, 1, 2, 3, 4, 5, 6);
    ...
    xit('returns node when there is a loop 2', function() {
      helpers.push(this.list, 1, 2, 3, 4, 5, 6);
    ...
    xit('returns node when there is a loop 3', function() {
      helpers.push(this.list, 1, 2, 3, 4, 5, 6);
